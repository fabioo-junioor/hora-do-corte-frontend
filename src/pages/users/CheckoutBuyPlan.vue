...
const checkoutVerify = () => {
    if(store.getters.getStateBuyPlan.pkPlan == null || store.getters.getStateBuyPlan.pkUser == null){
      ...
      <q-card dark flat class="my-card bg-brown-9">
        ...
        <q-card-section>
          ...
          <div class="text-subtitle2 wrap q-my-md">
            Beneficios:
            <q-badge
              v-for="i in dataBuyPlan.details[0]?.benefits" :key="i"
              class="q-ma-xs q-pa-xs text-white text-subtitle2" 
              color="brown-5" :label="i" />
          </div>
        </q-card-section>
